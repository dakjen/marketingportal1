...
function AdminReportGenerator({ fetchWordReports, wordReports }) {
  ...
  const handleGenerateReport = async () => {
    ...
    const prompt = `please ustilize the entries from social media and physical marketing to generate a report of: money spent, organizations worked with, total in each category, and total in each subcategory, as well as a short narrative at gthe very top describing all of the types of marketing hit and an approximate reach for each entry, as well as total reach per category. please utilize time frame for the physical marketing entry cateogires as well as details given to estimate reach. the estimated total reach should not be for the report timeframe, it should be the estimated total reach for the amount of time that is stated ont he entry that the item will be posted (for insteance some entires are 4 weeks, some are a month, etc). approximate the reach of each avenue via the entries and the "length of time" included on the entires use any and all notes available as well. give an approximation per item of possible reach in the time range, and also include an approximate reach/viewership total for all avenues within the length of time. the structure of this report should be title, summary, then a "short summary" section where you summarize the total reach, and total items added within the time frame, do not include costs, just the extimated total reach, then put the rest of the items named above: format it simply with bullets and narratives, like a plain word doc, no formatting indicators. Here is a summary of what has happened in within the dates: ${summary}. After the data, please provide 1-2 paragraphs of analysis on the performance compared to industry standards and to other affordable housing projects. you can give kind hearted and easy suggestions, but please don't hive any heavy handed critiques or new techniques. give 1-3 bullets on small ways we can hieghten enagegement. other than that this is simply a place to discuss wins, progress, and current status`;
    ...
    <div className="admin-report-generator-page-container">
      <div className="report-generator-section">
        ...
        <div className="saved-reports-section">
          <h3>Saved Reports</h3>
          {wordReports.filter(report => report.report_type === 'admin').length > 0 ? (
            <table>
              <thead>
                <tr>
                  <th>Report Name</th>
